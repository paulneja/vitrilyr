...
Open Preferences with `vitrilyr settings` or Shift+F7 where global shortcuts
...
`vitrilyr recover`, also available as Restore access in Behavior, restores a
...
vitrilyr seek -10
vitrilyr volume 50
vitrilyr layout normal
vitrilyr layout line
vitrilyr layout square
vitrilyr style contrast
vitrilyr language en
vitrilyr language es
vitrilyr language zh
vitrilyr move
vitrilyr recover
...
`vitrilyr --help` for available subcommands. `status` prints JSON including
...
configuration, with a one-time `config.kdl.before-vitrilyr` backup. On X11,
...
```sh
vitrilyr export-config ~/vitrilyr-settings.json
vitrilyr import-config ~/vitrilyr-settings.json
```
...
Settings live in `$XDG_CONFIG_HOME/vitrilyr/config.json`, normally
`~/.config/vitrilyr/config.json`. Missing fields receive defaults. Invalid JSON
...
Cache is under `$XDG_CACHE_HOME/vitrilyr/`, normally `~/.cache/vitrilyr/`.
